...
namespace WmsHub.Business.Services
{
  public class ApiKeyService : ServiceBase<Entities.Referral>, IApiKeyService
  {
    ...
    {
      _mapper = mapper;
      _access = ProviderOptions.Access;
    }
      ...
      bool validateProviders = false)
    {
      ...
      KeyValuePair<Guid, String> kvp =  await ValidateProviderKeyAsync(key);
      ...
      response = new ApiKeyStoreResponse();
      response.Sid = kvp.Key.ToString();
      response.Domain = (int)_access;
      response.Key = key;
      response.KeyUser = "provider";
      response.SetStatus(ValidationType.Valid, "");
      response.NameIdentifier = kvp.Value;
      return response;
    }
    ...
    public virtual async Task<KeyValuePair<Guid, string>> ValidateProviderKeyAsync(
      string key)
    {
      if (string.IsNullOrWhiteSpace(key))
      ...
      return provider == null 
        ? new KeyValuePair<Guid, string>( Guid.Empty, string.Empty )
        : new KeyValuePair<Guid, string>(provider.Id, provider.Name );
    }
