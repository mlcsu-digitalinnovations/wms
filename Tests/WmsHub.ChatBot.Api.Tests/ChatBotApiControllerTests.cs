...
public class ChatBotApiControllerTests : ReferralCallController
{
  ...
  private static readonly Mock<IMapper> s_mockMapper = new();
  private static readonly Mock<IOptions<ArcusOptions>> s_mockOptions = new();
  private readonly Mock<ArcusOptions> _mockSettings = new();

  private static readonly Mock<IArcusClientHelper> s_mockClientHelper = new();

  private readonly Mock<ChatBotService> _mockChatBotService;

  private readonly dynamic _transfer = new JObject();
  private readonly Mock<UpdateReferralTransferRequest> _mockTransferRequest =
    new("test", "+447123700001", (DateTimeOffset)DateTime.UtcNow);
  ...
  public ChatBotApiControllerTests() 
    : base(s_mockClientHelper.Object, null, s_mockOptions.Object, s_mockMapper.Object)
  {
    _mockSettings.Object.ReturnLimit = 600;
    _mockSettings.Object.ContactFlowName = CONTACT_FLOW_NAME;
    s_mockOptions.Setup(x => x.Value).Returns(_mockSettings.Object);

    _mockChatBotService = new(_mockContext.Object, s_mockOptions.Object, null);
  }

  protected override void LogInformation(string message) => Assert.True(true, message);

  protected override ChatBotService Service => _mockChatBotService.Object;
  ...
  public class GetTests : ChatBotApiControllerTests
  {
    public GetTests()
    {
      ...
      s_mockOptions.Setup(t => t.Value).Returns(mockOptions.Object);
    ...
    public async Task Valid_Result_Return_OK()
    {
      ...
      s_mockClientHelper.Setup(t => t.BatchPost(It.IsAny<ArcusCall>()))
    ...
    public async Task InvalidStatus_Returns_BadRequest()
    {
      ...
      List<string> errors = new() { "Test Error" };
      mockResponse.Setup(t => t.Errors).Returns(errors);
      ...
      // Act.
      IActionResult response = await Get();
