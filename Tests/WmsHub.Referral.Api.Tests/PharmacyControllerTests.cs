...
namespace WmsHub.Referral.Api.Tests
{
  ...
  public class PharmacyControllerTests : ServiceTestsBase, IDisposable
  {
    ...
    private static List<string> _validTemplateVersions = new() 
      {"1.0", "1.5", "2.0"};
    ...
    public class CreateAsync : PharmacyControllerTests
    {
      ...
      [Theory]
      [MemberData(nameof(GetInvalidPharmacyCreationData))]
        ...
        string odsCode, string templateVersion, string detail)
      {
        ...
        {
          Email =Generators.GenerateNhsEmail(_random),
          OdsCode = odsCode,
      ...
      [Theory]
      [MemberData(nameof(GetValidPharmacyCreationData))]
      [System.Diagnostics.CodeAnalysis.SuppressMessage("Usage",
      "xUnit1026:Theory methods should use all of their parameters",
      Justification = "<Pending>")]
      public async Task ValidCreate(
        string email, string name, string odsCode, string templateVersion)
      {
        // ARRANGE
      ...
      "xUnit1026:Theory methods should use all of their parameters",
      Justification = "<Pending>")]
      public async Task InValidOdsCreate(
        string email, string name, string odsCode, string templateVersion)
      {
        // ARRANGE
      ...
      public static IEnumerable<object[]> GetValidPharmacyCreationData()
      {
        List<object[]> validData = new();
        for (int i = 0; i < _validOdsCodeFirstPharmacyChars.Count; i++)
        {
          validData.Add(new object[] {
            Generators.GenerateNhsEmail(_random),
            Generators.GenerateName(_random, 10),
            Generators.GeneratePharmacyOdsCode(_random),
            _validTemplateVersions[0]
          });
        }
        return validData;
      }

      public static IEnumerable<object[]> GetInvalidPharmacyCreationData()
      {
        List<object[]> invalidData = new();
    ...
    public class UpdateAsync : PharmacyControllerTests
    {
      ...
      [Theory]
      [MemberData(nameof(GetValidPharmacyUpdateData))]
      [System.Diagnostics.CodeAnalysis.SuppressMessage("Usage",
      "xUnit1026:Theory methods should use all of their parameters",
      Justification = "<Pending>")]
      ...
      public static IEnumerable<object[]> GetValidPharmacyUpdateData()
      {
        List<object[]> validData = new();
        for (int i = 0; i < _validTemplateVersions.Count; i++)
        {
          validData.Add(new object[] {
            Generators.GenerateNhsEmail(_random),
            Generators.GenerateName(_random, 10),
            _existingPharmacy.OdsCode,
            _validTemplateVersions[i]
          });
        }
        return validData;
      }

      public static IEnumerable<object[]> GetInvalidPharmacyUpdateData()
      {
        List<object[]> invalidData = new();
        invalidData.Add(new object[]
        {
          ...
          _validTemplateVersions[0],
          $"Unable to find a pharmacy with an OdsCode of " +
          $"{EXISTING_PHARMACY_ODSCODE_NOT_FOUND}."
        });
        return invalidData;
      }
      ...
      string templateVersion = null)
    {
      return new()
      {
        Email = email ?? Generators.GenerateNhsEmail(_random),
        IsActive = isActive,
    ...
    public static IEnumerable<object[]> GetInvalidOdsCodeData()
    {
      ...
      string templateVersion = _validTemplateVersions[0];

      List<object[]> invalidData = new() {
        new object[] { null, templateVersion, required },
          new object[] { "", templateVersion, required }
        };
