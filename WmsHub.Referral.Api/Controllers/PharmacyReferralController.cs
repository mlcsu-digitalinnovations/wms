  ...
  public class PharmacyReferralController : BaseController
  {
      ...
      [FromBody] IsNhsNumberInUseRequest request)
    {
      try
      {
        ...
        var result = await Service.IsNhsNumberInUseAsync(request.NhsNumber);
        ...
        else if (result.InUseResult.HasFlag(InUseResult.Cancelled))
        {
          ...
          if (result.InUseResult.HasFlag(InUseResult.ProviderSelected))
          {
            ...
            Log.Debug("NHS number {nhsNumber} was previously used " +
              "with a referral that had selected a provider.",
              request.NhsNumber);
            return Problem($"NHS number {request.NhsNumber} was previously " +
              $"used with a referral that had selected a provider.",
              statusCode: StatusCodes.Status409Conflict);
          }
        ...
        {
          // No
          Log.Debug("NHS number {nhsNumber} is already in use.",
            request.NhsNumber);
          return Problem($"NHS number {request.NhsNumber} is already in use.",
            statusCode: StatusCodes.Status409Conflict);
        }
      }
      catch (Exception ex)
