...
namespace WmsHub.Referral.Api.Controllers
{
  ...
  public class ReferralExceptionController : BaseController
  {
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    [ProducesResponseType(StatusCodes.Status503ServiceUnavailable)]
    [Route("MissingAttachment/{ubrn}")]
    public async Task<IActionResult> PutMissingAttachment(string ubrn)
    {
      IReferralExceptionUpdate update = new ReferralExceptionUpdate
      {
        Ubrn = ubrn,
      ...
      ReferralNhsNumberMismatchPost model)
    {
      IReferralExceptionUpdate update =
        _mapper.Map<ReferralExceptionUpdate>(model);
    ...
    [ProducesResponseType(StatusCodes.Status503ServiceUnavailable)]
    [Consumes("application/json")]
    [Route("InvalidAttachment/{ubrn}")]
    public async Task<IActionResult> PutInvalidAttachment(string ubrn)
    {
      IReferralExceptionUpdate update = new ReferralExceptionUpdate
      {
        Ubrn = ubrn,
        ExceptionType = CreateReferralException.InvalidAttachment
      };
      ...
      IReferralExceptionUpdate update)
    {
      try
      {
        ...
        IReferral response = await Service
          .UpdateReferralToStatusExceptionAsync(update);
