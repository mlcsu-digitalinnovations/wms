...
{
  public abstract class AReferralPostRequest : IValidatableObject
  {
    ...
    {
      if (!Ethnicity.TryParseToEnumName<Business.Enums.Ethnicity>(out _))
      {
        yield return new InvalidValidationResult(nameof(Ethnicity), Ethnicity);
      }
      if (!Sex.TryParseToEnumName<Business.Enums.Sex>(out _))
      {
