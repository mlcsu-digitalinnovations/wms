...
public partial class ReferralAttachmentPdfProcessor
{
  ...
  /// <summary>
  /// Empty constructor required for simplified mocking
  /// </summary>
  ...
  public virtual async Task<bool> DownloadFile(
    ...
    )
  {
    ...
    try
    {
      _log.Information("A006-start-call");
      HttpResponseMessage response = await client
        .GetAsync($"{attachmentPath}{attachment.Url}");
      if (response.IsSuccessStatusCode)
      {
        _log.Information("A006-success-call");
        ...
        catch (Exception ex)
        {
          ...
          throw new ProcessDocumentException(ex.Message, ex);
        }
      }
      else
      {
    ...
    catch (Exception ex)
    {
      if (ex is IncorrectFileTypeException)
      {
        _log.Error($"Error in A006 process: {ex.Message}", ex);
        throw;
      }
      else if (ex is ProcessDocumentException)
      {
        _log.Error($"Error in ProcessDocument: {ex.Message}", ex);
        throw;
      }
      else
      {
        _log.Error("When trying to download the attachment from" +
        $" {attachmentPath}{attachment.Url} there was an" +
        " exception thrown by the client: " + ex.Message, ex);
        throw new FhirA006ClientException(
          "When trying to download the attachment from" +
          $" {attachmentPath}{attachment.Url} there was an" +
          " exception thrown by the client: " + ex.Message, ex);
      }
    }
    ...
    string[] sectionHeadings)
  {
    ...
    {
      if (fileExtension.ToUpperInvariant() == "RTF")
      {
        RtfPreprocessor rtfPreprocessor = new(Config.RtfPreprocessorConfig, _log);
        int numberOfIssues = rtfPreprocessor.ScanRtfFile(new MemoryStream(content));
      ...
      Processor processor = new(_log);
      InteropResult interopResult = await processor.ConvertToPdfInteropAsync(
        content, 
        fileExtension, 
        tempFilePath, 
        reformatDocument,
      ...
      {
        _log.Debug("Export Error converting to PDF");
        ExportFailed = true;
        InteropFailed = false;
        return false;
      }
    ...
    DateTimeOffset? mostRecentAttachmentDate)
  {
    ...
    ReferralPut result = new ReferralPut()
    {
      ...
      VulnerableDescription = 
        DocumentValue("Vulnerable Reason:", 1000, true),
      Mobile = 
    ...
    string referralLetterDate = DocumentValue("Referral Date:", 200);
    DateTimeOffset? referralLetterDateResult =
      ConvertToDateTimeOffset("Referral Date:", referralLetterDate);
    result.ReferralLetterDate = referralLetterDateResult;
    ...
    DateTimeOffset? mostRecentAttachmentDate)
  {
    ...
    else
    {
      ...
      result = new ReferralPost
      {
        ...
        ReferringGpPracticeNumber = DocumentValue("Practice Code:", 200),
        Address3 = DocumentValue("Address 3:", 200, true),
        VulnerableDescription = DocumentValue(
          "Vulnerable Reason:", 
          200, 
          true),
        Mobile =
    ...
    string attachmentId)
  {
    ...
    Console.WriteLine($"ReferralLetterDate|" +
      $"{referralCreate.ReferralLetterDate}");
    ...
    if (Downloaded == true)
    {
      ...
      Console.WriteLine($"ReferralLetterDate|" +
        $"{referralUpdate.ReferralLetterDate}");
      Console.WriteLine($"ReferringGpPracticeName|" +
