...
namespace IdentityAgentApi
{
  public static class GaTicket
  {
    ...
    {      
      ETicketReturnCodes eTicketReturnCode = (ETicketReturnCodes)returnCode;
      string result;

      if (Enum.IsDefined(eTicketReturnCode))
      {
        result = $"Return Code: '{eTicketReturnCode}'.";
      }
      else
      {
        result = $"Not Processed: Unknown Return Code '{returnCode}'.";
      }        
