...
namespace WmsHub.Utilities.Seeds
{
  public class ReferralSeeder : SeederBase<Referral>
  {
    ...
    public static async Task<List<Guid>> DeleteTestData()
    {
      string TEST_UBRN_PREFIX = Config["SeedSettings:Ubrn_Prefix"];

      List<Guid> ProvidersToDelete = new List<Guid>();
      try
      {
        List<Referral> referralToCheck = await DatabaseContext.Referrals
          .AsNoTracking()
          .Where(t => t.DateOfReferral > DateTimeOffset.Now.AddYears(-75))
          .ToListAsync();

        List<Referral> ReferralsToDelete = new List<Referral>();

        foreach (var refToCheck in referralToCheck)
        {
          var ubrn = refToCheck.Ubrn;
          if (ubrn.StartsWith(TEST_UBRN_PREFIX))
          {
            ReferralsToDelete.Add(refToCheck);
            if (refToCheck.ProviderId != null)
              ProvidersToDelete.Add(refToCheck.ProviderId.Value);
          }
        }

        if (ReferralsToDelete.Count > 0)
        {
          DatabaseContext.Referrals.RemoveRange(ReferralsToDelete.ToArray());
        }
      }
      catch (Exception)
      {
      ...
      Serilog.Log.Information("Deleted Referrals");
    ...
    internal static int CreateOneReferralForEachStatus()
    {
      ...
      {
        Referral referral = RandomEntityCreator.CreateRandomReferral(
          status: rs, triagedCompletionLevel: "1");
      ...
      DateTimeOffset? dateOfReferral = null)
    {
      ...
      dateOfReferral = dateOfReferral ?? DateTimeOffset.Now
        .AddDays(-random.Next(86, 90));
      ...
      for (int i = 0; i < noOfRecords; i++)
      {
        ...
        string ubrn = testUbrnPrefix
          .Substring(0, testUbrnPrefix.Length - idxLength) + (i + 1).ToString();
    ...
    public static void SeedDevelopmentData()
    {
      ...
      AddRmcCallTwoTextMessagesTwoCalls();
    }
    ...
    private static void AddChatBotCall2()
    {
      Referral chatBotCall2 = RandomEntityCreator.CreateRandomReferral(
        status: ReferralStatus.ChatBotCall2);

      TextMessage txt1 = RandomEntityCreator.CreateRandomTextMessage(
        number: chatBotCall2.Mobile,
        sent: DateTimeOffset.Now.AddHours(-98),
        modifiedByUserId: TextMessageSeeder.GOVUKNOTIFY_API_USER_ID);
      ...
      chatBotCall2.TextMessages = new List<TextMessage>()
      { txt1, txt2 };
      ...
      chatBotCall2.Calls = new List<Call>() { call1, call2 };

      DatabaseContext.Referrals.Add(chatBotCall2);
      DatabaseContext.SaveChanges();
    }
    ...
    private static void AddRmcCallTwoTextMessagesTwoCalls()
    {
      Referral rmcCallTwoTextMessagesTwoCalls =
        RandomEntityCreator.CreateRandomReferral(
          status: ReferralStatus.RmcCall);

      TextMessage txt1 = RandomEntityCreator.CreateRandomTextMessage(
        number: rmcCallTwoTextMessagesTwoCalls.Mobile,
        sent: DateTimeOffset.Now.AddHours(-49),
        modifiedByUserId: TextMessageSeeder.GOVUKNOTIFY_API_USER_ID);
      ...
      rmcCallTwoTextMessagesTwoCalls.TextMessages = new List<TextMessage>()
      { txt1, txt2 };
      ...
      rmcCallTwoTextMessagesTwoCalls.Calls = new List<Call>()
      { call1, call2 };

      DatabaseContext.Referrals.Add(rmcCallTwoTextMessagesTwoCalls);
      DatabaseContext.SaveChanges();
    }
  }
}